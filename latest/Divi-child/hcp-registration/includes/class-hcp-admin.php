...
class HCP_Admin {
    ...
    public static function handle_approve() {
        ...
        update_user_meta( $user_id, 'hcp_pharmacy_name', sanitize_text_field( $request->pharmacy_name ) );
    ...
    public static function handle_trade_approve() {
        ...
        if ( $existing_user ) {
			...
			$existing_user->remove_role( 'healthcare_professional' );
        } else {
    ...
    public static function render_user_profile_fields( $user ) {
        ...
        $pharmacy_name  = get_user_meta( $user->ID, 'hcp_pharmacy_name', true );
        $hcp_type       = get_user_meta( $user->ID, 'hcp_type', true );
        $hcp_reg_number = get_user_meta( $user->ID, 'hcp_reg_number', true );

        if ( in_array( 'healthcare_professional', $user_roles, true ) || $phone || $pharmacy_name || $hcp_type || $hcp_reg_number ) {
            ?>
            ...
            <table class="form-table" role="presentation">
                ...
                <tr>
                    <th><label for="hcp_pharmacy_name"><?php esc_html_e( 'Pharmacy/Clinic Name', 'hcp-registration' ); ?></label></th>
                    <td><input type="text" name="hcp_pharmacy_name" id="hcp_pharmacy_name" value="<?php echo esc_attr( $pharmacy_name ); ?>" class="regular-text" /></td>
                </tr>
                <tr>
        ...
        if ( in_array( 'trade_account', $user_roles, true ) || $trade_trading_name ) {
            $trade_fields = array(
                'trade_pharmacy_name'          => __( 'Pharmacy/Clinic Name', 'hcp-registration' ),
    ...
    public static function save_user_profile_fields( $user_id ) {
        ...
        // HCP fields.
        $hcp_meta = array( 'hcp_phone', 'hcp_pharmacy_name', 'hcp_type', 'hcp_reg_number' );
        ...
        // Trade fields.
        $trade_meta = array(
            'trade_pharmacy_name', 'trade_hcp_type', 'trade_hcp_reg_number',
